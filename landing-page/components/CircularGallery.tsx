...
function createTextTexture(
  ...
  const lineHeight = Math.ceil(baseFontSize * 1.1);

  // parse font to extract family (very small parser, expects something like 'bold 30px Family')
  const fontMatch = font.match(/(?:(bold|normal|bolder|lighter)\s+)?(\d+)px\s+(.+)/i);
  const fontWeight = fontMatch && fontMatch[1] ? fontMatch[1] : 'normal';
  const fontFamily = fontMatch && fontMatch[3] ? fontMatch[3] : 'sans-serif';

  // measure max width across lines using per-line font choices (name bold, subject smaller)
  let maxWidth = 0;
  for (let i = 0; i < lines.length; i++) {
    const isName = i === 0;
    const size = isName ? baseFontSize : Math.max(12, Math.round(baseFontSize * 0.8));
  ...
  // Re-apply defaults after resize
